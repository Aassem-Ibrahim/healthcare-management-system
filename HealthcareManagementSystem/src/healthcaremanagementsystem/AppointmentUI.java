...
public class AppointmentUI extends javax.swing.JFrame {
    public AppointmentUI() {
        this(true, new Appointment());  /* Enable edit for new appointment */
    }
    ...
    private int FormValidity() {
        ...
        if (appoint.index == -1) return FORM_E_DB;
    ...
    private void setEnableForComponentButton(Component[] cmp, boolean flag) {
        ...
        for (Component component : cmp) {
            if (component instanceof JButton) { /* Check component is jButton */
                component.setEnabled(flag);     /* Set enable of jButton */
            }
        }
    }
    ...
    private void setDateTime(String time, String Midday, String date) {
        ...
        try {
            ...
            if (!time.equals("") && !date.equals("")) {
                ...
                if (Midday.equals("AM")) rbAM.doClick();
                else rbPM.doClick();
    ...
    private void formatHours() {
        ...
        /* Add leading zeros */
        tfHours.setText("0".repeat(length) + tfHours.getText());
    }
    private void formatMinutes() {
        ...
        /* Add leading zeros */
        tfMinutes.setText("0".repeat(length) + tfMinutes.getText());
    }
    ...
    private void initComponents() {
        ...
        dateChooser.setDateFormatString("EEEE d MMM, y");
        dateChooser.setMaxSelectableDate(new java.util.Date(253370761291000L));
        dateChooser.setNextFocusableComponent(cbDoctors);
    ...
    private void bSaveActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_bSaveActionPerformed
        ...
        switch(FormValidity()) {
            case FORM_E_OK -> saveForm();
            case FORM_E_DB -> showErrorSaving(EMSG_DB);
            case FORM_E_TIME -> showErrorSaving(EMSG_TIME);
            case FORM_E_DATE_FORMAT -> showErrorSaving(EMSG_DATE_FORMAT);
            case FORM_E_DATE_VALID -> showErrorSaving(EMSG_DATE_VALID);
            case FORM_E_NO_DOCTOR -> showErrorSaving(EMSG_NO_DOCTOR);
            case FORM_E_NO_PATIENT -> showErrorSaving(EMSG_NO_PATIENT);
        }
    ...
    private void tfHoursKeyTyped(java.awt.event.KeyEvent evt) {//GEN-FIRST:event_tfHoursKeyTyped
        ...
        if ((tfHours.getText().trim().length() == 2
            ...
            tfHours.selectAll();            /* Select all */
            evt.consume();                  /* Consume Event */
        }
    ...
    private void tfMinutesKeyTyped(java.awt.event.KeyEvent evt) {//GEN-FIRST:event_tfMinutesKeyTyped
        ...
        if ((tfMinutes.getText().trim().length() == 2
                && (curCaret.getMark() - curCaret.getDot()) == 0)
                || evt.getKeyChar() == java.awt.event.KeyEvent.VK_BACK_SPACE) {
            tfMinutes.selectAll();          /* Select all */
            evt.consume();                  /* Consume Event */
        }
    ...
    private void cbDoctorsMousePressed(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_cbDoctorsMousePressed
        ...
        if (!chbEdit.isSelected()) {
            jlTitle.requestFocus();     /* Remove focus from jComboBox */
            cbDoctors.hidePopup();      /* Hide popup menu */
        }
    ...
    private void cbPatientsMousePressed(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_cbPatientsMousePressed
        ...
        if (!chbEdit.isSelected()) {
            jlTitle.requestFocus();     /* Remove focus from jComboBox */
            cbPatients.hidePopup();     /* Hide popup menu */
        }
    ...
    private void tfMinutesFocusGained(java.awt.event.FocusEvent evt) {//GEN-FIRST:event_tfMinutesFocusGained
        tfMinutes.selectAll();              /* Select minutes text */
    ...
    private void tfMinutesFocusLost(java.awt.event.FocusEvent evt) {//GEN-FIRST:event_tfMinutesFocusLost
        formatMinutes();                    /* Add leading zeros */
